...
def ridge(w,xTr,yTr,lmbda):
    ...
    #xw_y = np.dot(xTr, w) - yTr
    #loss = 1/n * np.dot(xw_y.transpose(), xw_y) + lmbda * np.dot(w.transpose(), w)
    #xTx = np.dot(xTr.transpose(), xTr)
    #gradient = 2* np.dot(xTx, w) - 2 * np.dot(xTr.transpose(), yTr) + 2 * lmbda * w
    loss = 0
    gradient = np.zeros(w.shape)
    for i in range(n):
        temp = np.dot(w.transpose(), xTr[i]) - yTr[i]
        loss += temp**2
        gradient += 2 * temp * xTr[i]
...
xTr_test = np.array([[1, 2, 3], [-1, -2, -3]])
yTr_test = np.array([1, -1])
lmbda_test = 2

print np.dot(w_test.transpose(), w_test)

xw_y = np.dot(xTr_test, w_test) - yTr_test

print np.dot(xw_y.transpose(), xw_y)

ridge(w_test, xTr_test, yTr_test, lmbda_test)
...
def adagrad(func,w,alpha,maxiter,delta=1e-02):
    ...
    z = np.zeros(w.shape, dtype = float)
    loss, gradient = func(w)
    count = 0
    while np.linalg.norm(gradient) >= delta and count < maxiter:
        ...
        losses[count] = loss
        count += 1
        z += np.power(gradient, 2) + eps*np.ones(w.shape)
        w_new = w - alpha * np.divide(gradient, np.sqrt(z))
...
_, d = xTr.shape
w, losses = adagrad(lambda weight: ridge(weight, xTr, yTr, lmbda), np.random.rand(d), 1, 1000)
plt.figure(figsize=(10,6))
...
#what should I return when only w and xTr given?
...
def logistic(w,xTr,yTr):
    ...
    gradient = np.zeros(w.shape)
    for i in range(n):
        exp = np.exp(-yTr[i] * np.dot(w.transpose(), xTr[i]))
        loss += np.log(1 + exp)
        gradient -= exp / (1 + exp) * yTr[i] * xTr[i]
    return loss, gradient
#</GRADED>
...
def hinge(w,xTr,yTr,lmbda):
    ...
    gradient = 0
    for i in range(n):
        temp = 1 - yTr[i]*np.dot(w.transpose(), xTr[i])
        loss += np.max([temp, 0])
        if temp > 0:
...
def trainspamfiltercomp(xTr, yTr):
    ...
    '''
    for i in lmbda:
        weight = np.random.rand(np.shape(xTr)[1])
        w, losses = adagrad(lambda weight: hinge(weight, xTr, yTr, i), np.random.rand(d), 1, 1000)
        preds = linclassify(w, xTr)
        acc = np.mean(preds==yTr)
        if acc > acc_best:
            w_best = w
            acc_best = acc
    w = w_best
    ...
    lmbda = 0.1
    weight = np.random.rand(np.shape(xTr)[1])
    w, losses = adagrad(lambda weight: hinge(weight, xTr, yTr, lmbda), np.random.rand(d), 1, 50)
